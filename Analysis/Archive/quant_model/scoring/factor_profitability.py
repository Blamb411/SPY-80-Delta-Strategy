"""
Profitability Factor Scoring
==============================
Scores stocks on profitability: Gross Margin, EBIT Margin, Net Margin,
ROE, ROA, ROIC. Higher is better for all sub-factors (sector-relative).
"""
...
def score_profitability(symbols: List[str], as_of_date: str,
                        sector_map: Dict[str, str],
                        db_path: str = config.DB_PATH) -> Dict[str, Dict]:
    """
    Compute Profitability factor scores for all symbols.
    ...
    metric_data = {}
    for metric_name, weight, higher_is_better in config.PROFITABILITY_SUB_FACTORS:
        rows = conn.execute(
