"""
EPS Revisions Factor Scoring
===============================
Scores stocks on analyst EPS estimate revisions from LSEG I/B/E/S data.
Higher is better (upward consensus moves = bullish).

Uses consensus change metrics computed from point-in-time EPSMean:
  eps_estimate_change_7d/30d/90d, last_earnings_surprise, estimate_dispersion
"""
...
FACTOR_NAME = "eps_revisions"


def is_available(db_path: str = config.DB_PATH) -> bool:
    """Check if EPS revision data exists in the database."""
    conn = db_schema.get_connection(db_path)
    row = conn.execute(
        """SELECT COUNT(*) as cnt FROM fundamental_data
           WHERE metric_name = 'eps_estimate_change_30d' LIMIT 1"""
    ).fetchone()
    conn.close()
    return row["cnt"] > 0 if row else False


def score_eps_revisions(symbols: List[str], as_of_date: str,
                        sector_map: Dict[str, str],
                        db_path: str = config.DB_PATH) -> Dict[str, Dict]:
    """
    Compute EPS Revisions factor scores for all symbols.

    Returns empty results if LSEG data is not available.
    ...
    metric_data = {}
    for metric_name, weight, higher_is_better in config.EPS_REVISIONS_SUB_FACTORS:
        rows = conn.execute(
