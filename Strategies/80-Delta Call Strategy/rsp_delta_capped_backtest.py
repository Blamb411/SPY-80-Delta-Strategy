"""
Delta-Capped 80-Delta Call Strategy Backtest - RSP Version
===========================================================
Same strategy as SPY version but applied to RSP (Equal-Weight S&P 500 ETF).

RSP options may have lower liquidity, so this version includes liquidity filtering:
...
Usage:
    python rsp_delta_capped_backtest.py              # With liquidity filters
    python rsp_delta_capped_backtest.py --midpoint   # No filters, midpoint execution
    python rsp_delta_capped_backtest.py --no-filter  # No filters, buy at ask/sell at bid
"""
...
log = logging.getLogger("rsp_delta_capped")
...
# Share holdings (adjusted for RSP price)
SHARES = 3500              # Number of RSP shares held
...
CC_DTE_MAX = 60
CC_PT = 0.50
CC_MAX_CONTRACTS = 35      # Adjusted for RSP
...
def run_delta_capped_simulation(client, bars_by_date, trading_dates, vix_data,
    ...
    """Daily portfolio simulation with delta-capped options for RSP."""
...
def print_results(metrics, use_midpoint=False, no_filter=False):
    ...
    print(f"\n{'=' * W}")
    print(f"RSP DELTA-CAPPED 80-DELTA CALL STRATEGY -- BACKTEST RESULTS")
    print(f"{'=' * W}")
...
def main():
    parser = argparse.ArgumentParser(description="RSP Delta-Capped 80-Delta Call Strategy Backtest")
    ...
    if not monthly_exps:
        print("\nWARNING: No monthly expirations found for RSP.")
        print("RSP may have limited options history in ThetaData.")
        client.close()
        return
    ...
    if trades:
        if use_midpoint:
            ...
            suffix = ""
        output_file = os.path.join(_this_dir, f"rsp_trades{suffix}.csv")
