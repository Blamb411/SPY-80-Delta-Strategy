"""
Delta-Capped 80-Delta Call Strategy Backtest - QQQ Version
===========================================================
Same strategy as SPY version but applied to QQQ (Nasdaq-100 ETF).

Uses VXN (Nasdaq-100 Volatility Index) as IV proxy - the correct volatility
index for QQQ, analogous to how VIX is used for SPY.

Usage:
    python qqq_delta_capped_backtest.py
"""
...
log = logging.getLogger("qqq_delta_capped")
...
# Share holdings (adjusted for QQQ price vs SPY)
SHARES = 2000              # Number of QQQ shares held
...
CC_DTE_MAX = 60
CC_PT = 0.50
CC_MAX_CONTRACTS = 20      # Adjusted for QQQ
...
def load_all_data(client):
    ...
    print("Loading VXN history (Nasdaq-100 Volatility Index)...")
    vxn_data = client.fetch_volatility_index("^VXN", DATA_START, DATA_END)
    if not vxn_data:
        print("  WARNING: VXN not available, falling back to VIX...")
        vxn_data = client.fetch_vix_history(DATA_START, DATA_END)
    ...
    return bars_by_date, trading_dates, vxn_data, sma200, monthly_exps, trailing_12m_returns, rolling_volatility
...
def run_delta_capped_simulation(client, bars_by_date, trading_dates, vxn_data,
    ...
    """Daily portfolio simulation with delta-capped options for QQQ."""
    ...
    for day_idx in range(start_idx, len(trading_dates)):
        ...
        vxn = vxn_data.get(today, 20.0)
        iv_est = max(0.08, min(0.90, vxn / 100.0))
...
def print_results(metrics):
    ...
    print(f"\n{'=' * W}")
    print(f"QQQ DELTA-CAPPED 80-DELTA CALL STRATEGY -- BACKTEST RESULTS")
    print(f"{'=' * W}")
...
def main():
    ...
    bars_by_date, trading_dates, vxn_data, sma200, monthly_exps, trailing_12m_returns, rolling_volatility = load_all_data(client)

    # Run backtest
    snaps, trades, cc_trades = run_delta_capped_simulation(
        client, bars_by_date, trading_dates, vxn_data, sma200, monthly_exps,
    ...
    m = compute_metrics(snaps, trades, cc_trades, "Thresh-exit (no CC)")
    print_results(m)

    # Save trade log for analysis
    if trades:
        output_file = os.path.join(_this_dir, "qqq_trades.csv")
