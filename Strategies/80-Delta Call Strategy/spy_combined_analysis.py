"""
SPY Combined Strategy Analysis
===============================
Models a combined portfolio of:

  Component 1: $1M SPY Buy & Hold (shares held throughout)
  Component 2: $1M leveraged 80-delta call strategy
      - 50% profit target, no stop-loss, 60td max hold
      - Force-exit all when SPY >2% below SMA200
      - Idle cash in BIL (short-term treasuries)

Compares two versions:
  A) Without covered calls
  B) With covered calls sold on SPY shares during below-SMA200 periods
...
Usage:
    python spy_combined_analysis.py
"""
...
log = logging.getLogger("spy_combined")
...
OPTIONS_CAPITAL = 100_000   # sim runs at $100K, we scale by 10x
SCALE = ALLOC / OPTIONS_CAPITAL

DELTA = 0.80                # options strategy delta
DTE_TARGET = 120
DTE_MIN = 90
DTE_MAX = 150
MH = 60                     # max hold trading days
PT = 0.50                   # +50% profit target
RATE = 0.04                 # risk-free rate
SMA_EXIT_THRESHOLD = 0.02   # 2% below SMA200 force-exit

# Covered call parameters
CC_DELTA = 0.30             # sell 30-delta OTM calls
CC_DTE_TARGET = 35          # ~35 DTE
CC_DTE_MIN = 20
CC_DTE_MAX = 50
...
# ======================================================================
# HELPERS (same as spy_deployment_sim)
# ======================================================================
...
# ======================================================================
# OPTIONS STRATEGY SIMULATION (Config B with 3% threshold)
# ======================================================================

def run_options_sim(client, spy_by_date, trading_dates, vix_data, sma200,
                    monthly_exps):
    """
    Run the leveraged options strategy:
    - Buy 1 contract/day when SPY > SMA200
    - Force-exit all when >3% below SMA200
    - 50% PT, 60td MH, no SL
    """
    ...
    print(f"\n{'='*70}")
    print(f"Options Strategy: {DELTA:.0%}-delta calls, {SMA_EXIT_THRESHOLD:.0%} SMA exit")
    ...
    for day_idx in range(start_idx, len(trading_dates)):
        ...
        # Force-exit when >3% below SMA200
...
def run_cc_overlay(client, spy_by_date, trading_dates, vix_data, sma200,
                   monthly_exps, n_cc_contracts, sim_start):
    """
    Simulate selling covered calls on SPY shares during below-SMA200 periods.

    - Sell ~30-delta calls, ~35 DTE monthly expiration
    - Active when SPY < SMA200
    - Close when: expiration settles, or SPY recovers above SMA200

    Returns:
        daily_cc_values: list of (realized_cash, unrealized_mtm) per day
        cc_trade_log: list of CC trade dicts
    """
    cc_position = None   # {expiration, strike, n_contracts, entry_price, entry_date}
    cc_realized = 0.0    # cumulative cash from CC trades
    contract_eod = {}    # cached option data
    strikes_cache = {}
    cc_trade_log = []
    ...
    print(f"\n{'='*70}")
    print(f"Covered Call Overlay: {CC_DELTA:.0%}-delta, ~{CC_DTE_TARGET}d DTE")
    print(f"  Active when SPY < SMA200  |  {n_cc_contracts} contracts")
    print(f"{'='*70}")
    ...
    for day_idx in range(len(trading_dates)):
        ...
        # 1. Check if active CC has expired
        if cc_position:
            ...
            if today_dt >= exp_dt:
                ...
                # We lose the intrinsic (shares called away, rebuy at market)
                settlement = intrinsic * 100 * cc_position["n_contracts"]
                cc_realized -= settlement

                cc_trade_log.append({
                    "entry_date": cc_position["entry_date"],
                    "exit_date": today,
        ...
        # 2. If above SMA and have CC: buy back
        ...
        # 3. If below SMA and no CC: sell new one
            ...
            if cc_exp:
                ...
                if bs_strike:
                    ...
                    if strikes:
                        ...
                        if ckey not in contract_eod:
                            data = client.prefetch_option_life(
                                "SPY", cc_exp, real_strike, "C", today
                            )
                            contract_eod[ckey] = {r["bar_date"]: r for r in data}
                        eod = contract_eod[ckey].get(today)
                        bid, _ = get_bid_ask(eod)
        ...
        if cc_position:
            ...
            if bid and ask:
                ...
                mid = black_scholes_price(spot, cc_position["strike"], t_left,
                                          RATE, iv_est, "C") or 0
            # Short position: liability = -(mid * 100 * n)
...
def build_portfolios(snapshots, bil_rets, spy_by_date, cc_values, sim_start):
    """
    Construct daily portfolio values for with/without covered calls.

    Returns: dict with:
      dates, spy_bh, options_raw, options_w_bil,
      combined_no_cc, combined_w_cc, cc_contribution
    """
    dates = [s["date"] for s in snapshots]

    # SPY B&H ($1M)
    start_price = spy_by_date[dates[0]]["close"]
    n_shares = int(ALLOC // start_price)
    leftover = ALLOC - n_shares * start_price
    spy_bh = np.array([n_shares * spy_by_date[d]["close"] + leftover for d in dates])

    # Options component ($1M = 10x the $100K sim)
    opt_raw = np.array([s["portfolio_value"] for s in snapshots]) * SCALE

    # BIL yield on options idle cash
    cum_yield = 0.0
    opt_w_bil = []
    for s in snapshots:
        cash = s["cash"]
    ...
    # CC contribution -- cc_values covers all trading dates (from DATA_START),
    # but snapshots only start at SIM_START. Trim to match snapshot length.
    n_snap = len(dates)
    cc_trimmed = cc_values[-n_snap:]  # last N entries align with sim period
    cc_contrib = np.array([realized + unrealized for realized, unrealized in cc_trimmed])

    # Combined portfolios
    combined_no_cc = spy_bh + opt_w_bil
    combined_w_cc = spy_bh + opt_w_bil + cc_contrib
    ...
    return {
        ...
        "opt_raw": opt_raw,
        "opt_w_bil": opt_w_bil,
        "combined_no_cc": combined_no_cc,
        "combined_w_cc": combined_w_cc,
        "cc_contrib": cc_contrib,
        "n_shares": n_shares,
    }
...
def print_results(port, cc_trade_log, opt_trade_log, snapshots, spy_by_date):
    W = 105
    dates = port["dates"]
    years = len(dates) / 252.0

    print(f"\n\n{'=' * W}")
    print("SPY COMBINED STRATEGY ANALYSIS")
    print(f"{'=' * W}")
    print(f"  Period:     {dates[0]} to {dates[-1]} ({years:.1f} years)")
    print(f"  Component 1: $1M SPY B&H ({port['n_shares']:,} shares)")
    print(f"  Component 2: $1M leveraged {DELTA:.0%}-delta calls "
          f"({PT:.0%} PT, {MH}td MH, {SMA_EXIT_THRESHOLD:.0%} SMA exit)")
    print(f"  Cash:        Idle cash in BIL (short-term treasuries)")
    print(f"  Covered calls: {CC_DELTA:.0%}-delta OTM, ~{CC_DTE_TARGET}d DTE, monthly")
    print(f"                 Active when SPY < SMA200")

    # Component summaries
    s_spy = compute_stats(port["spy_bh"], "SPY B&H ($1M)")
    ...
    components = [s_spy, s_opt, s_no_cc, s_w_cc]
    ...
    # Main comparison: with vs without CC
    print(f"\n{'-' * W}")
    ...
    print(f"  {'Metric':<25} {'Without CC':>20} {'With CC':>20} {'Delta':>12}")
    print(f"  {'-' * 80}")
    ...
    print(f"\n  {'Year':<6} {'--- Without CC ---':^36} {'--- With CC ---':^36} {'SPY B&H':>8}")
    ...
    if sig_periods and cc_trade_log:
        tdf = pd.DataFrame(cc_trade_log)
        print(f"\n  {'Period':<30} {'Days':>6} {'CC Trades':>10} {'CC P&L':>12} {'SPY Move':>10}")
        print(f"  {'-' * 75}")

        for s, e in sig_periods:
            days_in = len([x for x in snapshots if s <= x["date"] <= e])
            spy_s = spy_by_date.get(s, {}).get("close", 0)
            spy_e = spy_by_date.get(e, {}).get("close", spy_s)
            spy_move = spy_e / spy_s - 1 if spy_s > 0 else 0

            cc_in = tdf[(tdf["entry_date"] >= s) & (tdf["entry_date"] <= e)]
            cc_pnl = cc_in["total_pnl"].sum() if len(cc_in) > 0 else 0
            n_cc = len(cc_in)

            print(f"  {s} to {e:<14} {days_in:>6} {n_cc:>10} ${cc_pnl:>+10,.0f} {spy_move:>+9.1%}")
    ...
    print(f"  CC added {s_w_cc['cagr'] - s_no_cc['cagr']:+.1%} CAGR, "
          f"{s_w_cc['sharpe'] - s_no_cc['sharpe']:+.2f} Sharpe")

    print(f"\n  SPY B&H reference: ${s_spy['start']:,.0f} -> ${s_spy['end']:,.0f}, "
...
def main():
    ...
    W = 80
    print("=" * W)
    print("SPY Combined Strategy: B&H + Leveraged Options + Covered Calls")
    print("=" * W)
    ...
    # Compute number of CC contracts
    ...
    port = build_portfolios(snapshots, bil_rets, spy_by_date, cc_values, SIM_START)

    # Print results
    print_results(port, cc_trade_log, opt_trade_log, snapshots, spy_by_date)
