"""
QQQ 80-Delta Call Deployment Simulator
=======================================
Models actual deployment of the systematic QQQ call strategy with
real ThetaData bid/ask pricing:

  - $100K starting capital, no margin
  - Buy 1 contract per day when QQQ > 200 SMA
...
Compares two SMA200 modes:
  ...
  B) Threshold exit: force-exit all positions when QQQ drops >2% below SMA200
...
log = logging.getLogger("qqq_deploy")
...
SMA_EXIT_THRESHOLD = 0.02  # force-exit when QQQ >2% below SMA200
...
def load_all_data(client):
    ...
    print("Loading SPY bars (for B&H comparison)...")
...
def run_simulation(client, qqq_by_date, trading_dates, vix_data, sma200,
                   monthly_exps, force_exit_below_sma=False, label=""):
    """
    Daily portfolio simulation.

    force_exit_below_sma: if True, force-exit all positions when QQQ drops
    ...
    for day_idx in range(start_idx, len(trading_dates)):
        ...
        # 2a. Force-exit all positions when QQQ >2% below SMA200
...
def compute_metrics(snapshots, trade_log, spy_by_date, label=""):
    ...
    # QQQ B&H (primary benchmark)
    qqq_start = df["qqq_close"].iloc[0]
    qqq_end = df["qqq_close"].iloc[-1]
    qqq_total = qqq_end / qqq_start - 1
    qqq_cagr = (qqq_end / qqq_start) ** (1 / years) - 1 if years > 0 else 0
    df["qqq_ret"] = df["qqq_close"].pct_change().fillna(0)
    qqq_sharpe = (df["qqq_ret"].mean() / df["qqq_ret"].std()) * np.sqrt(252) if df["qqq_ret"].std() > 0 else 0
    qqq_dd = (df["qqq_close"] / df["qqq_close"].cummax() - 1).min()

    # SPY B&H (secondary reference)
    sim_dates = df["date"].tolist()
    spy_closes = []
    for d in sim_dates:
        if d in spy_by_date:
            spy_closes.append(spy_by_date[d]["close"])
        elif spy_closes:
            spy_closes.append(spy_closes[-1])
        else:
            spy_closes.append(0)
    df["spy_close"] = spy_closes

    spy_start = df["spy_close"].iloc[0]
    spy_end = df["spy_close"].iloc[-1]
    spy_total = spy_end / spy_start - 1 if spy_start > 0 else 0
    spy_cagr = (spy_end / spy_start) ** (1 / years) - 1 if years > 0 and spy_start > 0 else 0
    ...
    spy_dd = (df["spy_close"] / df["spy_close"].cummax() - 1).min() if spy_start > 0 else 0
    ...
    for year in sorted(df["year"].unique()):
        ...
        qqq_y_start = ydf["qqq_close"].iloc[0]
        qqq_y_end = ydf["qqq_close"].iloc[-1]
        qqq_y_ret = qqq_y_end / qqq_y_start - 1
        ...
        yearly[year] = {
            "ret": y_ret, "qqq_ret": qqq_y_ret, "alpha": y_ret - qqq_y_ret,
    ...
    return {
        ...
        "qqq_total": qqq_total, "qqq_cagr": qqq_cagr,
        "qqq_sharpe": qqq_sharpe, "qqq_dd": qqq_dd,
...
def print_comparison(m_a, m_b):
    ...
    print(f"\n{'=' * W}")
    print("QQQ 80-DELTA CALL DEPLOYMENT SIMULATION -- COMPARISON")
    print(f"{'=' * W}")
    print(f"  Period:    {SIM_START} to {DATA_END}")
    print(f"  Capital:   ${CAPITAL:,} (no margin)")
    print(f"  Strategy:  Buy 1 contract/day when QQQ > SMA200")
    ...
    rows = [
        ...
        ("QQQ CAGR (ref)",    f"{m_a['qqq_cagr']:>+12.1%}", f"{m_b['qqq_cagr']:>+12.1%}"),
        ("QQQ Sharpe (ref)",  f"{m_a['qqq_sharpe']:>12.2f}", f"{m_b['qqq_sharpe']:>12.2f}"),
    ]
    ...
    for year in all_years:
        ...
        qqq_r = ya.get("qqq_ret", yb.get("qqq_ret", 0))

        print(f"  {year:<6}  {ra:>+7.1%} {aa:>+7.1%} {ta_n:>7} {ma:>7}  "
              f"{rb:>+7.1%} {ab:>+7.1%} {tb_n:>7} {mb:>7}  {qqq_r:>+6.1%}")
    ...
    print(f"\n  QQQ B&H: CAGR {m_a['qqq_cagr']:+.1%}  |  "
          f"Sharpe: {m_a['qqq_sharpe']:.2f}  |  Max DD: {m_a['qqq_dd']:.1%}")
    print(f"  SPY B&H: CAGR {m_a['spy_cagr']:+.1%}  |  "
...
def main():
    ...
    print("=" * 80)
    print("QQQ 80-Delta Call -- Deployment Simulation (v3)")
    print("=" * 80)
    ...
    # Run B: threshold exit when QQQ >2% below SMA200
    ...
    m_a = compute_metrics(snaps_a, trades_a, spy_by_date, "A: Entry-only SMA filter")
    m_b = compute_metrics(snaps_b, trades_b, spy_by_date, "B: Thresh-exit (>2% below SMA)")
