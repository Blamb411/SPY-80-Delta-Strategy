...
"""
Multi-Ticker Options Spread Test - BATCH VERSION
=================================================

Identical strategy to multi_ticker_spread_test.py but processes tickers
in batches of 3 to avoid QuantConnect resource exhaustion.

Run this algorithm 4 times, changing ACTIVE_BATCH each time:
  Batch 1: SPY, QQQ, IWM
  Batch 2: DIA, XLF, XLE
  Batch 3: XLK, XLV, GLD
  Batch 4: SLV, TLT, HYG

After all 4 runs, combine the CSV output from each run's debug log.
The on_end_of_algorithm() prints CSV-formatted trade data for easy merging.

Changes from the original:
  - ACTIVE_BATCH controls which 3 tickers to process
  - Strike filter narrowed from (-30,+30) to (-15,+15) to reduce universe
  - CSV trade export in debug log for offline combination
  - All strategy logic is IDENTICAL to the original
"""


# =============================================================================
# BATCH CONFIGURATION - CHANGE THIS FOR EACH RUN
# =============================================================================

ACTIVE_BATCH = 1  # Set to 1, 2, 3, or 4

ALL_TICKERS = ["SPY", "QQQ", "IWM", "DIA", "XLF", "XLE", "XLK", "XLV",
               "GLD", "SLV", "TLT", "HYG"]
...
TICKERS = BATCHES[ACTIVE_BATCH]


# =============================================================================
# STRATEGY CONFIGURATION (identical to original)
# =============================================================================
...
class MultiTickerBatchTest(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.debug("=" * 70)
        self.debug(f"MULTI-TICKER BATCH TEST - BATCH {ACTIVE_BATCH} of 4")
    ...
    # =========================================================================
    # ENTRY LOGIC (identical to original)
    # =========================================================================
    ...
    def _build_entry(self, ticker: str, sd: SymbolData, iv_rank: float, data):
        ...
        if created_any:
            ...
            if self.entry_signal_count <= 200:
    ...
    def on_end_of_algorithm(self):
        ...
        self.debug("")
        self.debug("=" * 80)
        self.debug(f"BATCH {ACTIVE_BATCH} RESULTS — {', '.join(TICKERS)}")
        self.debug("=" * 80)
        self.debug(f"Period: {self.start_date.date()} to {self.end_date.date()}")
        ...
        for key, pnls in combo_results.items():
            ...
            total = sum(pnls)
            combo_stats.append({
                'strategy': strat, 'width': width, 'tp_pct': tp, 'sl_mult': sl,
                'count': count,
        ...
        self.debug("")
        self.debug("-" * 80)
        self.debug("BEST COMBINATION PER TICKER")
        self.debug("-" * 80)
        ...
        # =====================================================================
        # CSV EXPORT — copy this from the debug log to combine batches
        # =====================================================================
        self.debug("")
        self.debug("=" * 80)
        self.debug("CSV_START")
        self.debug("ticker,strategy,width,tp_pct,sl_mult,entry_date,exit_date,"
                   "expiry,entry_credit,actual_width,short_strike,long_strike,"
                   "spot_at_entry,exit_reason,exit_pnl,peak_pnl,trough_pnl")
        ...
        for t in sorted(trades, key=lambda x: str(x['entry_date'])):
            ...
            exit_d = t['exit_date'].date() if t['exit_date'] and hasattr(t['exit_date'], 'date') else t['exit_date']
            exp_d = t['expiry'].date() if hasattr(t['expiry'], 'date') else t['expiry']
            self.debug(f"{t['ticker']},{t['strategy']},{t['width']},"
                       f"{t['tp_pct']},{t['sl_mult']},"
                       f"{entry_d},{exit_d},{exp_d},"
                       f"{t['entry_credit']:.4f},{t['actual_width']},"
                       f"{t['short_strike']},{t['long_strike']},"
                       f"{t['spot_at_entry']:.2f},{t['exit_reason']},"
                       f"{t['exit_pnl']:.2f},{t['peak_pnl']:.2f},{t['trough_pnl']:.2f}")

        self.debug("CSV_END")
        self.debug("=" * 80)
        self.debug(f"Total CSV rows: {len(trades)}")
        self.debug(f"Batch {ACTIVE_BATCH} complete. Run batches {[b for b in range(1,5) if b != ACTIVE_BATCH]} next.")
    ...
    def _print_csv_header(self):
        """Print CSV header even if no trades, for consistency."""
        self.debug("CSV_START")
        self.debug("ticker,strategy,width,tp_pct,sl_mult,entry_date,exit_date,"
                   "expiry,entry_credit,actual_width,short_strike,long_strike,"
                   "spot_at_entry,exit_reason,exit_pnl,peak_pnl,trough_pnl")
